...
static const appExtAdvCfg_t fitExtAdvCfg = {
    { 0, 0, 0 }, /*! Advertising durations for extended advertising in ms */
    { 60 / 0.625, 60 / 0.625,
      60 / 0.625 }, /*! Advertising intervals for extended advertising in 0.625 ms units (20 ms to 10.24 s). */
    { 0, 0,
...
static const appUpdateCfg_t fitUpdateCfg = {
    ...
    150 / 1.25, /*! Maximum connection interval in 1.25ms units */
...
static const uint8_t fitExtAdvDataDisc[] = {
    /*! flags */
    2, /*! length */
...
static const uint8_t fitExtScanDataDisc[] = {
    /*! device name */
    4, /*! length */
    DM_ADV_TYPE_LOCAL_NAME, /*! AD type */
    'F', 'i', 't'
...
static void fitSetup(fitMsg_t *pMsg)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
    	...
    	AppExtAdvSetData(i, APP_ADV_DATA_DISCOVERABLE, sizeof(fitExtAdvDataDisc),
    				(uint8_t *)fitExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    	AppExtAdvSetData(i, APP_SCAN_DATA_DISCOVERABLE, sizeof(fitExtScanDataDisc),
    				(uint8_t *)fitExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
	
    	/* set advertising and scan response data for connectable mode */
    	AppExtAdvSetData(i, APP_ADV_DATA_CONNECTABLE, sizeof(fitExtAdvDataDisc),
    					(uint8_t *)fitExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    	AppExtAdvSetData(i, APP_SCAN_DATA_CONNECTABLE, sizeof(fitExtScanDataDisc),
                     	(uint8_t *)fitExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
	}
...
void FitHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    APP_TRACE_INFO6("MAC Addr: %02x:%02x:%02x:%02x:%02x:%02x", addr[5], addr[4], addr[3], addr[2],
                    addr[1], addr[0]);
    APP_TRACE_INFO1("Adv local name: %s", &fitExtScanDataDisc[2]);
