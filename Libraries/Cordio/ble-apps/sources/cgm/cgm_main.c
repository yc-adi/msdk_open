 ...
 *  \file
 *
 *  \brief  Cgmness sample application for the following profiles:
 *            Heart Rate profile
 *
...
#define CGM_MSG_START               0xA0

/* Default Running Speed and Cadence Measurement period (seconds) */
#define CGM_DEFAULT_RSCM_PERIOD        1

/*! WSF message event enumeration */
enum
{
  CGM_HR_TIMER_IND = CGM_MSG_START,       /*! Heart rate measurement timer expired */
  CGM_BATT_TIMER_IND,                     /*! Battery measurement timer expired */
  CGM_RUNNING_TIMER_IND                   /*! Running speed and cadence measurement timer expired */
};
...
static const hrpsCfg_t cgmHrpsCfg =
{
  2000      /*! Measurement timer expiration period in ms */
};

/*! battery measurement configuration */
static const basCfg_t cgmBasCfg =
{
...
enum
{
  CGM_GATT_SC_CCC_IDX,                    /*! GATT service, service changed characteristic */
  CGM_HRS_HRM_CCC_IDX,                    /*! Heart rate service, heart rate monitor characteristic */
  CGM_BATT_LVL_CCC_IDX,                   /*! Battery service, battery level characteristic */
  CGM_RSCS_SM_CCC_IDX,                   /*! Runninc speed and cadence measurement characteristic */
  CGM_NUM_CCC_IDX
};

/*! client characteristic configuration descriptors settings, indexed by above enumeration */
static const attsCccSet_t cgmCccSet[CGM_NUM_CCC_IDX] =
{
  /* cccd handle          value range               security level */
  {GATT_SC_CH_CCC_HDL,    ATT_CLIENT_CFG_INDICATE,  DM_SEC_LEVEL_NONE},   /* CGM_GATT_SC_CCC_IDX */
  {HRS_HRM_CH_CCC_HDL,    ATT_CLIENT_CFG_NOTIFY,    DM_SEC_LEVEL_NONE},   /* CGM_HRS_HRM_CCC_IDX */
  {BATT_LVL_CH_CCC_HDL,   ATT_CLIENT_CFG_NOTIFY,    DM_SEC_LEVEL_NONE},   /* CGM_BATT_LVL_CCC_IDX */
  {RSCS_RSM_CH_CCC_HDL,   ATT_CLIENT_CFG_NOTIFY,    DM_SEC_LEVEL_NONE}    /* CGM_RSCS_SM_CCC_IDX */
};
...
/*! WSF handler ID */
wsfHandlerId_t cgmHandlerId;

/* WSF Timer to send running speed and cadence measurement data */
wsfTimer_t     cgmRscmTimer;

/* Running Speed and Cadence Measurement period - Can be changed at runtime to vary period */
static uint16_t cgmRscmPeriod = CGM_DEFAULT_RSCM_PERIOD;

/* Heart Rate Monitor feature flags */
static uint8_t cgmHrmFlags = CH_HRM_FLAGS_VALUE_8BIT | CH_HRM_FLAGS_ENERGY_EXP;
 ...
 */
/*************************************************************************************************/
static void cgmAttCback(attEvt_t *pEvt)
{
  attEvt_t *pMsg;
...
*/
/*************************************************************************************************/
static void cgmSendRunningSpeedMeasurement(dmConnId_t connId)
{
  if (AttsCccEnabled(connId, CGM_RSCS_SM_CCC_IDX))
  {
  ...
  cgmRscmTimer.msg.event = CGM_RUNNING_TIMER_IND;
  cgmRscmTimer.msg.status = CGM_RSCS_SM_CCC_IDX;
  cgmRscmTimer.handlerId = cgmHandlerId;
  cgmRscmTimer.msg.param = connId;

  WsfTimerStartSec(&cgmRscmTimer, cgmRscmPeriod);
}
...
static void cgmProcCccState(cgmMsg_t *pMsg)
{
  ...
  if (pMsg->ccc.idx == CGM_HRS_HRM_CCC_IDX)
  {
    if (pMsg->ccc.value == ATT_CLIENT_CFG_NOTIFY)
    {
      HrpsMeasStart((dmConnId_t) pMsg->ccc.hdr.param, CGM_HR_TIMER_IND, CGM_HRS_HRM_CCC_IDX);
    }
    else
    {
  ...
  if (pMsg->ccc.idx == CGM_RSCS_SM_CCC_IDX)
  {
    if (pMsg->ccc.value == ATT_CLIENT_CFG_NOTIFY)
    {
      cgmSendRunningSpeedMeasurement((dmConnId_t)pMsg->ccc.hdr.param);
    }
    else
    {
      WsfTimerStop(&cgmRscmTimer);
    }
    return;
  }

  /* handle battery level CCC */
  if (pMsg->ccc.idx == CGM_BATT_LVL_CCC_IDX)
  {
    if (pMsg->ccc.value == ATT_CLIENT_CFG_NOTIFY)
    {
      BasMeasBattStart((dmConnId_t) pMsg->ccc.hdr.param, CGM_BATT_TIMER_IND, CGM_BATT_LVL_CCC_IDX);
    }
    else
    {
 ...
 */
/*************************************************************************************************/
static void cgmClose(cgmMsg_t *pMsg)
{
 ...
 */
/*************************************************************************************************/
static void cgmSetup(cgmMsg_t *pMsg)
{
  ...
  if ((connId = AppConnIsOpen()) != DM_CONN_ID_NONE)
  {
    switch (btn)
    {
      ...
      case APP_UI_BTN_2_MED:
        ...
        if (!(cgmHrmFlags & (CH_HRM_FLAGS_SENSOR_DET | CH_HRM_FLAGS_SENSOR_NOT_DET)))
        {
          cgmHrmFlags |= CH_HRM_FLAGS_SENSOR_DET;
        }
        else if (cgmHrmFlags & CH_HRM_FLAGS_SENSOR_DET)
        {
          cgmHrmFlags &= ~CH_HRM_FLAGS_SENSOR_DET;
          cgmHrmFlags |= CH_HRM_FLAGS_SENSOR_NOT_DET;
        }
        else
        {
          cgmHrmFlags &= ~CH_HRM_FLAGS_SENSOR_NOT_DET;
        ...
        HrpsSetFlags(cgmHrmFlags);
        break;

      case APP_UI_BTN_2_LONG:
        /* Toggle HRM RR Interval feature flag */
        if (cgmHrmFlags & CH_HRM_FLAGS_RR_INTERVAL)
        {
          cgmHrmFlags &= ~CH_HRM_FLAGS_RR_INTERVAL;
        }
        else
        {
          cgmHrmFlags |= CH_HRM_FLAGS_RR_INTERVAL;
  ...
  else
  {
    switch (btn)
    {
      ...
      case APP_UI_BTN_2_SHORT:
        ...
        if (cgmHrmFlags & CH_HRM_FLAGS_VALUE_16BIT)
        {
          cgmHrmFlags &= ~CH_HRM_FLAGS_VALUE_16BIT;
        }
        else
        {
          cgmHrmFlags |= CH_HRM_FLAGS_VALUE_16BIT;
 ...
 */
/*************************************************************************************************/
static void cgmProcMsg(cgmMsg_t *pMsg)
{
  uint8_t uiEvent = APP_UI_NONE;

  switch(pMsg->hdr.event)
  {
    case CGM_RUNNING_TIMER_IND:
      cgmSendRunningSpeedMeasurement((dmConnId_t)pMsg->ccc.hdr.param);
      break;

    case CGM_HR_TIMER_IND:
      HrpsProcMsg(&pMsg->hdr);
      break;

    case CGM_BATT_TIMER_IND:
      BasProcMsg(&pMsg->hdr);
      break;
...
void CgmHandlerInit(wsfHandlerId_t handlerId)
{
  ...
  HrpsInit(handlerId, (hrpsCfg_t *) &cgmHrpsCfg);
  HrpsSetFlags(cgmHrmFlags);

  /* initialize battery service server */
  BasInit(handlerId, (basCfg_t *) &cgmBasCfg);
}
...
void CgmStart(void)
{
  ...
  AttRegister(cgmAttCback);
  AttConnRegister(AppServerConnCback);
  AttsCccRegister(CGM_NUM_CCC_IDX, (attsCccSet_t *) cgmCccSet, cgmCccCback);
  ...
  GattSetSvcChangedIdx(CGM_GATT_SC_CCC_IDX);
