...
void lctrLlcpStatelessEventHandler(lctrConnCtx_t *pCtx, uint8_t event)
{
  switch (event)
  {
    ...
    case LCTR_CONN_LLCP_START_PENDING:
      ...
      break;
    default:
      APP_TRACE_INFO2("Unhandled LLCP SM event, handle=%u, event=%u", LCTR_GET_CONN_HANDLE(pCtx), event);
      break;
  }
}
...
{
  APP_TRACE_INFO2("lctrSlvLlcpExecuteSm evt=%d(%s)", event, GetLctrEvtStr(event));
