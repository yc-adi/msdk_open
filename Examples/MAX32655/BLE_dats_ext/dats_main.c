...
static const appExtAdvCfg_t datsExtAdvCfg = {
    ...
    { 96, 96,
      96 }, /*! Advertising intervals for extended advertising in 0.625 ms units (20 ms to 10.24 s). */
    { 0, 0,
      0 }, /*! Maximum number of extended advertising events controller will send prior to terminating extended advertising */
    { TRUE, FALSE,
      FALSE }, /*! Whether to use legacy advertising PDUs with extended advertising. If set to TRUE then length of advertising data cannot exceed 31 octets. */
    { 0, 0,
      0 } /*! Advertising intervals for periodic advertising in 1.25 ms units (7.5 ms to 81.91875 s). */
};
 ...
 */
/*************************************************************************************************/
void datsStartAdv(void)
{
    uint8_t advHandles[3];
    advHandles[0] = 0;
    advHandles[1] = 1;
    advHandles[2] = 2;
...
static void datsPrivAddDevToResList(appDbHdl_t dbHdl)
{
    ...
    if ((pPeerKey = AppDbGetKey(dbHdl, DM_KEY_IRK, NULL)) != NULL) {
        ...
        AppExtSetAdvPeerAddr(0, pPeerKey->irk.addrType, pPeerKey->irk.bdAddr);
        AppExtSetAdvPeerAddr(1, pPeerKey->irk.addrType, pPeerKey->irk.bdAddr);
        AppExtSetAdvPeerAddr(2, pPeerKey->irk.addrType, pPeerKey->irk.bdAddr);
    }
}
...
static void datsPrivRemDevFromResListInd(dmEvt_t *pMsg)
{
    if (pMsg->hdr.status == HCI_SUCCESS) {
        if (AppDbGetHdl((dmConnId_t)pMsg->hdr.param) != APP_DB_HDL_NONE) {
            ...
            AppExtSetAdvPeerAddr(0, HCI_ADDR_TYPE_PUBLIC, addrZeros);
            AppExtSetAdvPeerAddr(1, HCI_ADDR_TYPE_PUBLIC, addrZeros);
            AppExtSetAdvPeerAddr(2, HCI_ADDR_TYPE_PUBLIC, addrZeros);
        }
    }
}
...
static void datsSetup(dmEvt_t *pMsg)
{
    ...
    AppExtAdvSetData(0, APP_ADV_DATA_DISCOVERABLE, sizeof(datsExtAdvDataDisc),
                     (uint8_t *)datsExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(0, APP_SCAN_DATA_DISCOVERABLE, sizeof(datsExtScanDataDisc),
                     (uint8_t *)datsExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(1, APP_ADV_DATA_DISCOVERABLE, sizeof(datsExtAdvDataDisc),
                     (uint8_t *)datsExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(1, APP_SCAN_DATA_DISCOVERABLE, sizeof(datsExtScanDataDisc),
                     (uint8_t *)datsExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(2, APP_ADV_DATA_DISCOVERABLE, sizeof(datsExtAdvDataDisc),
                     (uint8_t *)datsExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(2, APP_SCAN_DATA_DISCOVERABLE, sizeof(datsExtScanDataDisc),
                     (uint8_t *)datsExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);

    /* set advertising and scan response data for connectable mode */
    AppExtAdvSetData(0, APP_ADV_DATA_CONNECTABLE, sizeof(datsExtAdvDataDisc),
                     (uint8_t *)datsExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(0, APP_SCAN_DATA_CONNECTABLE, sizeof(datsExtScanDataDisc),
                     (uint8_t *)datsExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(1, APP_ADV_DATA_CONNECTABLE, sizeof(datsExtAdvDataDisc),
                     (uint8_t *)datsExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(1, APP_SCAN_DATA_CONNECTABLE, sizeof(datsExtScanDataDisc),
                     (uint8_t *)datsExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(2, APP_ADV_DATA_CONNECTABLE, sizeof(datsExtAdvDataDisc),
                     (uint8_t *)datsExtAdvDataDisc, HCI_EXT_ADV_DATA_LEN);
    AppExtAdvSetData(2, APP_SCAN_DATA_CONNECTABLE, sizeof(datsExtScanDataDisc),
                     (uint8_t *)datsExtScanDataDisc, HCI_EXT_ADV_DATA_LEN);
    ...
    // DmAdvSetPhyParam(uint8_t advHandle, uint8_t priAdvPhy, uint8_t secAdvMaxSkip, uint8_t secAdvPhy);
    DmAdvSetPhyParam(0, HCI_ADV_PHY_LE_1M, 0, HCI_ADV_PHY_LE_1M);
    DmAdvSetPhyParam(1, HCI_ADV_PHY_LE_CODED, 0, HCI_ADV_PHY_LE_CODED);
    DmAdvSetPhyParam(2, HCI_ADV_PHY_LE_1M, 0, HCI_ADV_PHY_LE_2M);
}
...
void DatsHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    APP_TRACE_INFO6("MAC Addr: %02x:%02x:%02x:%02x:%02x:%02x", addr[5], addr[4], addr[3], addr[2],
                    addr[1], addr[0]);
    APP_TRACE_INFO1("Adv local name: %s", &datsExtScanDataDisc[2]);
