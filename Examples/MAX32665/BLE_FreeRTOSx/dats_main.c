...
static struct {
	...
	uint8_t           status;             /* last notify status, most likely overflow */
	bool_t            pending;            /* notify pending */
} datsCb;
...
bool_t DatsIsConnected() {
	return datsCb.connected;
}

bool_t DatsIsIdle() {
	return !datsCb.pending && (datsCb.status != ATT_ERR_OVERFLOW);
}
...
static void datsProcMsg(dmEvt_t *pMsg)
{
    ...
    case DM_CONN_OPEN_IND:
		...
		datsCb.status = ATT_SUCCESS;
		datsCb.pending = FALSE;
		datsCb.connected = TRUE;
        uiEvent = APP_UI_CONN_OPEN;
        break;

    case ATTS_HANDLE_VALUE_CNF:
		datsCb.status = pMsg->hdr.status;
		datsCb.pending = FALSE;
		break;
...
uint8_t *buffer = (uint8_t *)"gnlkjhjydxuxmidxddgaqgvupmcukirzcsablyyfmthzfhkjzagnmzaarvzwncpaywagabpctguoyowvthsoehewvgazqfyghfaracarltjhitbzofxwcvdzahesndzopwffiizfotjudoxqutfwgvtlphwhqeecblpzqubanzivnbcyiigjzsvwnqrebvkzkimbqyh";
