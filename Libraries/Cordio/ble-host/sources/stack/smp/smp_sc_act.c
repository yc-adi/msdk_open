...
bool_t smpScProcPairing(smpCcb_t *pCcb, uint8_t *pOob, uint8_t *pDisplay)
{
  ...
  *pOob = SMP_OOB_DATA_NONE;
  ...
  {
    SMP_TRACE_INFO2("  LESC pCcb->pairReq/Rsp=8 (SMP_AUTH_SC_FLAG) ReqOOB=%d ResOOB=%d",
  ...
  {
    SMP_TRACE_INFO2("  legacy pairing ReqOOB=%d ResOOB=%d",
      pCcb->pairReq[SMP_OOB_POS], pCcb->pairRsp[SMP_OOB_POS]);
  ...
  {
    SMP_TRACE_INFO2("  MITM req=0x%x rsp=0x%x", pCcb->pairReq[SMP_AUTHREQ_POS], pCcb->pairRsp[SMP_AUTHREQ_POS]);
  ...
  if ((pCcb->pairReq[SMP_AUTHREQ_POS] & SMP_AUTH_SC_FLAG) &&
      (pCcb->pairRsp[SMP_AUTHREQ_POS] & SMP_AUTH_SC_FLAG))
  {
    SMP_TRACE_INFO0("  SMP_AUTH_SC_FLAG");
    if (smpCb.lescSupported == FALSE)
    {
      /* cancel pairing if LESC is not initialized by application */
      SMP_TRACE_INFO0("  cancel pairing since LESC is not initialized");
    ...
    if (SmpScAllocScratchBuffers(pCcb))
    {
      ...
      SMP_TRACE_INFO0("  evt 17 (SMP_MSG_INT_LESC)");
  ...
  else if ((pSmpCfg->auth & SMP_AUTH_SC_FLAG) != 0)
  {
    ...
    SMP_TRACE_INFO1("  auth=%d cancel pairing if cfg requires LESC and we are falling back to legacy", pSmpCfg->auth);
    hdr.param = pCcb->connId;
    hdr.status = SMP_ERR_AUTH_REQ;
    hdr.event = SMP_MSG_API_CANCEL_REQ;
    smpSmExecute(pCcb, (smpMsg_t *) &hdr);
  ...
  else
  {
    ...
    SMP_TRACE_INFO0("  evt=18 (SMP_MSG_INT_LEGACY)");
  ...
  else
  {
    ...
    pCcb->auth = pCcb->pairReq[SMP_AUTHREQ_POS] & pCcb->pairRsp[SMP_AUTHREQ_POS] & ~SMP_AUTH_MITM_FLAG;
    SMP_TRACE_INFO1("  auth=%d set auth flags with mitm bit cleared", pCcb->auth);
  }
